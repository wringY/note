## 如何打开CMD命令 ##
1. CMD命令：开始－>运行－>键入cmd或command(在命令行里可以看到系统版本、文件系统版本)。
2. 如何在当前文件目录中打开 CMD
  **按住 shift  + 鼠标右键** 
...
- notepad-记事本
- write -写字版
- sysdm.cpl-打开环境变量设置窗口，点击环境变量，在系统变量中找到path，点击新建，如添加一个E:\eudic。这样我们进入dos系统时就有了全局变量E:\eudic。我们可以直接输入eudic.exe,就能运行该软件了。
...
　　cdup 进入远程主机目录的父目录
　　! 在本地机中执行交互shell，exit回到ftp环境,如!ls*.zip*\#5
##FTP命令详解##
...
　　9.case：在使用mget时，将远程主机文件名中的大写转为小写字母。
     ...
     　　26.lcd[dir]：将本地工作目录切换至dir。
      　　11. ls[remote-dir][local-file]：显示远程目录remote-dir， 并存入本地文件local-file。
